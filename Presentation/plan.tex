...
\end{definition}\par
	\subsection{Logique de s\'eparation}
		\subsubsection{Syntaxe}
	La syntaxe de la logique de logique de s\'eparation (SL) est construite sur la syntaxe de FOL, d\'efinie \`a la subsection 1.1.1. Nous consid\'erons une th\'eorie du premier ordre $T=(\mathsmaller\sum,M)$ telle que $\sum^{s}$ contient les types $Loc$ et $Data$ et $\sum^{f}$ contient une constante $nil^{Loc}$.
	\begin{definition}
