...
\chapter{Logique du premier ordre}
	\section{Logique du premier ordre}
		...
		\begin{itemize}
		\item $\sum^s = \{ \sigma_{1}, \sigma_{2},...\}$ est un ensemble de symboles de type. On suppose l'existence du type $Bool = \{ \top , \bot \}$, o\`u nous \'ecrirons $\top$ et $\bot$ pour les constantes, respectivement, true et false.
	   ...
	   Posons $Var = {x,y,z,...}$ un ensemble d\'enombrable de variable du premi\`ere ordre. Chaque variable $x^{\sigma} \in Var$ est associ\'e \`a un type $\sigma \in \sum^{s}$.
...
~~~~~$|$~~ $t_{1} = t_{2},$~~~~~~~~~~~~~~~~~~~~~~~~$t_{1}^{\sigma},t_{2}^{\sigma}$ $terms$~~~~~~~~~~~~~~~~~~~~~~~~~~(\'egalit\'e)\par
~~~~~$|$~~$\phi^{FOL}$,~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~(n\'egation)\par
...
\end{definition}
Les constantes $\top$ et $\bot$, des termes bool\'een, et l'\'egalit\'e entre deux termes de m\^eme type sont des formules du premi\`ere ordre. La n\'egation, conjonction, disjonction, les quantificateurs existentiel et universelle de la logique du premi\`ere ordre sont aussi des formules de la logique du premi\`ere ordre.
\begin{definition}
...
\begin{definition}
(Taille d'une formule du premi\`ere ordre). La taille d'une formule $\phi$, not\'e $|\phi|$, est la somme des tailles de tous les termes dedans :
\end{definition}
